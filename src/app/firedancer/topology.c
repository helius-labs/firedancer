...
fd_topo_initialize( config_t * config ) {
  ...
  if( leader_enabled ) fd_topob_wksp( topo, "txsend" );
  ...
  if( leader_enabled ) fd_topob_wksp( topo, "net_txsend"   );
  ...
  if( leader_enabled ) fd_topob_wksp( topo, "txsend_out"    );
  ...
  if( leader_enabled ) fd_topob_wksp( topo, "txsend_sign"   );
  if( leader_enabled ) fd_topob_wksp( topo, "sign_txsend"   );
  ...
  if( leader_enabled ) fd_topob_link( topo, "txsend_net",    "net_txsend",    config->net.ingress_buffer_size,          FD_NET_MTU,                    1UL );
  ...
  if( leader_enabled ) fd_topob_link( topo, "txsend_sign",   "txsend_sign",   128UL,                                    FD_TXN_MTU,                    1UL ); /* TODO: Depth probably doesn't need to be 128 */
  if( leader_enabled ) fd_topob_link( topo, "sign_txsend",   "sign_txsend",   128UL,                                    sizeof(fd_ed25519_sig_t)*2UL,  1UL ); /* TODO: Depth probably doesn't need to be 128 */
  ...
  if( leader_enabled ) fd_topob_link( topo, "txsend_out",    "txsend_out",    128UL,                                    FD_TPU_RAW_MTU,                1UL );
  ...
  if(leader_enabled) FOR(net_tile_cnt) fd_topos_net_rx_link( topo, "net_txsend", i, config->net.ingress_buffer_size );
  ...
  if( leader_enabled ) fd_topob_tile( topo, "txsend",  "txsend",  "metric_in",  tile_to_cpu[ topo->tile_cnt ], 0,        1,                 0 );
  ...
  if(leader_enabled) FOR(net_tile_cnt) fd_topob_tile_in( topo, "txsend",  0UL,          "metric_in", "net_txsend",    i,            FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED ); /* No reliable consumers of networking fragments, may be dropped or overrun */
  ...
  if(leader_enabled) fd_topos_tile_in_net( topo,                          "metric_in", "txsend_net",    0UL,          FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED ); /* No reliable consumers of networking fragments, may be dropped or overrun */
  ...
  if(leader_enabled) fd_topob_tile_in (   topo, "gossip", 0UL,           "metric_in", "txsend_out",    0UL,          FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );
  ...
  if(leader_enabled) fd_topob_tile_in (   topo, "replay",  0UL,          "metric_in", "txsend_out",    0UL,          FD_TOPOB_RELIABLE,   FD_TOPOB_POLLED );
  ...
  /* Stream tile — streams executed transactions to Laserstream via gRPC.
     Follows the same pattern as solcap: one output link per execrp tile,
     consumed by a single stream tile.  All links are UNRELIABLE so the
     stream tile can never backpressure the execution pipeline. */

  int stream_enabled = config->tiles.stream.enabled;
  if( FD_UNLIKELY( stream_enabled ) ) {
    fd_topob_wksp( topo, "stream" );
    fd_topob_wksp( topo, "execrp_strm" );

    /* One link per execrp tile, depth 256, MTU 65535 (USHORT_MAX).
       256 × 65535 = 16 MiB per link.  Typical transactions are 3-5 KiB. */
    FOR(execrp_tile_cnt) fd_topob_link( topo, "execrp_strm", "execrp_strm", 256UL, USHORT_MAX, 1UL );

    fd_topob_tile( topo, "stream", "stream", "metric_in", tile_to_cpu[ topo->tile_cnt ], 0, 0, 0 );

    /* Wire execrp → stream.  Each execrp tile gets its own output link. */
    FOR(execrp_tile_cnt) fd_topob_tile_out( topo, "execrp", i, "execrp_strm", i );

    /* Stream tile reads from all execrp_strm links as UNRELIABLE consumer. */
    FOR(execrp_tile_cnt) fd_topob_tile_in( topo, "stream", 0UL, "metric_in", "execrp_strm", i, FD_TOPOB_UNRELIABLE, FD_TOPOB_POLLED );

    /* Stream tile also reads tower_out for slot status (confirmed/rooted)
       and replay_out for block completion events.  UNRELIABLE so we never
       backpressure tower or replay. */
...
fd_topo_configure_tile( fd_topo_tile_t * tile,
  ...
  if( FD_UNLIKELY( !strcmp( tile->name, "metric" ) ) ) {
    ...
    if( config->firedancer.layout.enable_block_production ) tile->net.txsend_src_port = config->tiles.txsend.txsend_src_port;
